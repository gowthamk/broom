...
than theirs. Moreover, our type system comes with a type
...
language is explicitly typed, with some support for local type
inference. Consequently, region types for methods and classes need to
be written explicitly. In contrast, we support full type inference
that eliminates such need.
...
not first-class values of the language. Nonetheless, due to aliasing
among region names, ~\cite{HMN01} requires reference counting for
regions to ensure memory safety.
