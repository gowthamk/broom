...
\lopsemrule{3.85in}{
    \allocRgn(\fbN) \in \rhoenv \spc
    \mbody(m,\fbN) = \rhobar.\bar{x}.e 
%   \redstoo{(, \rhomap)}{(e',\rhomap')}
}{
    \redstoo{((\C{new}\;\fbN(\bar{v})).m\inang{\rbar}
                      (\bar{v'}),\rhomap)}
            {([\rbar/\rhobar][\bar{v'}/\xbar][\C{new} \; \fbN(\bar{v})/\thisZ]\,e,\rhomap)}
	  }{METHOD-INV}

\lopsemrule{2.85in}{
    v_a = \lambdaexp{\rgn}{\rhobar}
                        {\taubar \; \xbar}{e} \spc
    \rgn \in \rhoenv \spc 
}{
    \redstoo{(v_a\inang{\rbar}(\bar{v}) ,\rhomap)}
            {([\bar{v}/\xbar][\rbar/\rhobar]\,e,\rhomap)}
	  }{FUN-APPLY}
...
\begin{smathpar}
\begin{array}{lcl}
E & \coloneqq & \bullet \ALT (\bullet).f \ALT \bullet.m\inang{\rbar}(\ebar) \ALT
      v.m\inang{\rbar}(...,\bullet,...) \ALT \C{new}\; \fbN(...,\bullet,...) \ALT
      \C{new} \; \RgnZ\inang{T}\inang{\toprgn}(\bullet) \ALT \bullet\inang{\rbar}(\ebar) \\
  &  & \ALT v\inang{\rbar}(...,\bullet,...) \ALT \open{\bullet}{\rgn}{y}{e}
\end{array}
\end{smathpar}
