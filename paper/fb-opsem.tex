...
%   \rgn \in dom(\mem) \spc
    ...
    \redstoo{(\C{new} \; \RgnZT{\toploc\loc} (e),\mem)}
            {\invalidexn}
	  }{EXCEPTION}
...
E & \coloneqq & \bullet \ALT (\bullet).f \ALT \bullet.m\inang{\locbar}(\ebar) \ALT
  ...
  &  & \ALT v\inang{\locbar}(...,\bullet,...) \ALT
       \letd{\loc}{\bullet} \ALT \open{\bullet}{\rgn}{y}{e} \ALT
       \opened{\loc}{\status}{\bullet} \ALT \letexp{x}{\bullet}{e}
...
%   \redstoo{(e,\mem)}{(e',\mem')}
    ...
    \redstoo{(\letregion{\rgn}{e},\mem)}{(\letd{\loc}{[\loc/\rgn]e},\mem')}
  }{LET-REGION-BEGIN}
...
\lopsemrule{3.3in}{
    \mem(\loc) = \USED \spc
    \redstoo{(e,\mem[\loc \mapsto \LIVE]) }{(e',\mem')}
}{
    \redstoo{(\C{new} \; \RgnZ\inang{T}\inang{\toploc\loc}
                (e),\mem)}
            {(\C{new} \; \RgnZ\inang{T}\inang{\toploc\loc}
...
%   \rgn_0 \notin \rhoenv \spc
    ...
    \redstoo{(\opened{\loc}{\status}{v},\mem)} {(v,\mem')}
  }{OPEN-END}
...
%   \redstoo{(, \mem)}{(e',\mem')}
    ...
    \redstoo{((\C{new}\;\RgnZ\inang{T}\inang{\toploc\loc}(v)).\transfer(),\mem)}
            {(\unitval,\mem')}
	  }{TRANSFER}
