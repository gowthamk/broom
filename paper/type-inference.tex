...
a sequence of region parameters $\pi_0, \cdots \pi_n$ that \C{C}
...
we replace every instance of class \C{C} in the program by an instance
$\C{C}\langle \rho_0, \cdots, \rho_n \rangle$, where $\rho_0, \cdots, \rho_n$
are fresh identifiers.

In particular, we determine the sequence of region parameters for a class \C{C}
by starting with the sequence of region parameters required for the type of
the base class of \C{C}, and adding fresh region parameters for the type of every field
in the class.
The class \C{Object} has a single region parameter $\pi_0$ (the allocation region).
...
% occurrences of a class type in its definition ensures that no new
