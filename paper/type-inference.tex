...
The solution $\soln$ to a set of constraints $C$ is a pair $(\regionSubstFn,\predSubstFn)$,
...
Fig.~\ref{fig:fb-staticsem} with symbols $\A$ and $\env$ retaining their meaning.
...
\end{theorem}
\begin{proof}
  We inductively establish a correspondence between derivation trees under
  the static semantics and the constraint-generation trees.

% We do not present a formal proof of the theorem.
The proof essentially requires establishing the following lemma inductively: if
$\exprok {\stdcontext}{e}{\tau}{C}$
and $\soln$ satisfies $C$, then
$\hastyp {\stdcontext}{e[\soln]}{\tau}$,
where $e[\soln]$ indicates $e$ with the substitutions
in the solution $\soln$, as well as analogous lemmas for type well-formedness.
\end{proof}
...
However, the entire type inference algorithm is not complete, because of some heuristic
