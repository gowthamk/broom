...
letregion $\rgn_0$ in
  ...
  letregion $\rgn_1$ in
    ...
    let p = new Pair<Object,Object><$\rgn_1$,$\rgn_1$,$\rgn_0$> (fst,snd);
\end{codejava}
%\vspace*{-0.1in}
...
\C{Pair} object contained in  $\rgn$. Separate parameterization over
...
Fig.~\ref{fig:fb-opsem-1} and~\ref{fig:fb-opsem-2}
in the appendix defines a small-step operational semantics for \fbname
via a five-place reduction relation:
% \vspace*{-0.15in}
\begin{smathpar}
  \redstoo{\rhoenv}{(e,\rhomap)}{(e',\rhomap')}
\end{smathpar}
