...
single region, it yeilds benefits that far outweigh the costs. In
...
The type rule for method application uses auxiliary definitions
...
current method as accessed via $\thisZ$ (i.e., $\thisZ.m$) is
region-parametric, thus admitting region-polymorphic recursion (i.e.,
recursive call to $m$ can have different region arguments than $m$). 
...
additional langauge constructs of \FB that manifest only at run-time.
...
that formalize the intuitions desribed above is shown in
