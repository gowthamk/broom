...
(\rgn'\in\Delta).~(\isvalid{\phicx}{\rgn\outlives\rgn'}) ~\Rightarrow~
\rgn = \rgn'$ ensures that the closure is always allocated in the
last-opened region (also called the \emph{current allocation context}).
This prevents the closure from escaping the context in which it is
created , thus trivially ensuring the safety of any dereferences
inside the closure.
