...
region parameters as $\rho\rhobar$ to clearly distinguish between
the allocation region parameter ($\rho$) and the rest.}.
%
...
parameterization separate from region parameterization allows \FB to
...
relationships on an eponymous region. Provided this is satisfied, the
expression ($e$) under \C{letregion} is then typechecked assuming that
the new region is live ($\Delta \cup \{\pi\}$) and that it is outlived
by all existing live regions ($\Delta \outlives \pi$). The result of a
\C{letregion} expression must have a type that is well-formed under a
context not containing the new region. This ensures that the value
obtained by evaluating a \C{letregion} expression contains no
references to the temporary objects inside the region.
...
extended with the function's region parameters ($\rho\rhobar$). Note
that the closure is always allocated in the current allocation context
($\rgn$). This prevents the closure from escaping the context in which
it is created, thus trivially ensuring the safety of any dereferences
inside the closure. 
